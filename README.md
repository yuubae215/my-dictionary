...
- [ ] 第M編 メタ語彙（M.01〜M.14、原辞書6.1節） — 未着手。**独立編とすることは決定済み**（文書番号 `STD-EED-0001-M`。`decisions.md` D-2026-08-22-19）
- [ ] 全編化時の附属書（MECE検証・索引類） — メタ語彙編の後に追加

**対象語彙（原辞書5章、45概念・現場語85語）の変換は完了。** 概念ドリフトの検出は45件中2件（第I部1.07、第IV部4.01）。

未決事項は `backlog.md` に集約している（判断待ち6件・提案値の確定待ち3件・原辞書へのフィードバック11件）。前提なしで着手できた2件は消化済みで、**残る判断待ちはすべて第M編（メタ語彙）の変換待ち**になった。次の作業単位は第M編の変換である。
