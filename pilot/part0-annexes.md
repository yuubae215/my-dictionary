...
**変換日:** 2026-09-03（最終更新 2026-09-05）
...
もう一つ、本編には索引ではない役割がある。**分類が漏れなく重なりなく（MECE）成立しているかを、全編を1つの母数として検証すること**である（附属書F）。各編の変換では自分の編の内側しか見えないため、「Layers 軸の6値すべてに語彙があるか」「どの分類の組み合わせが空か」は編をまたいで数え直さなければ答えられない。原辞書4.4節・4.5節がこの役割を担っており、本編はそれを68エントリの現状で作り直す。
...
| 第2編 空間統合 (Integration層) | STD-EED-0001-2 | 19 |
...
**現場語105語は、（語, 主語）の組で一意であり、各組はちょうど1つの代表形（概念）を持ちます。** この一意性は自動検証の対象です（原辞書4.4節、附属書F.4）。同義・異表記の列に載っている語も、同じ代表形へ畳み込まれます。

**2026-09-05 改定:** 従来の規則「各現場語はちょうど1つの代表形を持つ」は語単位の一意性であり、原辞書2.1節が同音異義の筆頭に挙げる「原点」「姿勢」「クリアランス」を登録できなかった——「同じ言葉が違う意味を持つ」問題を解くための辞書が、同じ言葉を複数の意味で登録する手段を持っていなかったことになる。一意性を（語, 主語）の組へ改めたことで、同じ語を主語違いで複数行に登録できる（`decisions.md` D-2026-09-05-04）。主語（誰について言っているか）が代表形を選ぶ。★は本仕様書で登録した現場語で、存在確認と発番は原辞書側の承認待ち（`backlog.md` P-06）。
...
| `eed:0154#001` ★ | **右** | 第2編3.18 direction reference frame | 作業者・オペレータ・保全担当 | 右側／向かって右 |
| `eed:0155#001` ★ | **左** | 第2編3.18 direction reference frame | 作業者・オペレータ・保全担当 | 左側／向かって左 |
| `eed:0156#001` ★ | **手前** | 第2編3.18 direction reference frame | 工作機械・専用機（CNC、プレス、溶接機など） | 前／フロント側 |
| `eed:0157#001` ★ | **奥** | 第2編3.18 direction reference frame | 工作機械・専用機（CNC、プレス、溶接機など） | 後ろ／リア側 |
| `eed:0158#001` ★ | **上流** | 第2編3.18 direction reference frame | コンベア・シュート | 前工程側／入側 |
| `eed:0159#001` ★ | **下流** | 第2編3.18 direction reference frame | コンベア・シュート | 後工程側／出側 |
| `eed:0160#001` ★ | **表・裏** | 第2編3.19 discrete orientation class | ワーク（被加工物・部品・製品） | 表向き／裏向き／表裏フラグ |
| `eed:0161#001` ★ | **正・逆** | 第2編3.19 discrete orientation class | ワーク（被加工物・部品・製品） | 正逆フラグ／向き反転 |
| `eed:0162#001` ★ | **向き不問** | 第2編3.19 discrete orientation class | 搬送トレー・パレット・コンテナ | Don't care／向きフリー |
| `eed:0163#001` ★ | **原点** | 第2編3.9 world space | CADシステム・PLM | 絶対原点／シーン原点 |
| `eed:0164#001` ★ | **原点** | 第2編3.14 datum reference feature | 治具（ジグ）・位置決めピン・突き当て面 | 突き当て原点／機械原点 |
| `eed:0165#001` ★ | **原点** | 第3編3.11 grasp specification and synthesis | エンドエフェクタ（ハンド・グリッパ・吸着パッド） | TCPオフセット／把持原点 |
| `eed:0166#001` ★ | **姿勢** | 第2編3.5 pose | ワーク（被加工物・部品・製品） | 向き／角度 |
| `eed:0167#001` ★ | **姿勢** | 第2編3.5 pose | 産業用ロボット（多関節マニピュレータ） | ツール姿勢／TCP姿勢 |
| `eed:0168#001` ★ | **姿勢** | 第2編3.19 discrete orientation class | PLC（シーケンサ） | 向きフラグ |
| `eed:0169#001` ★ | **クリアランス** | 第4編3.3 measurement | ワーク（被加工物・部品・製品） | 隙間／公差 |
| `eed:0170#001` ★ | **クリアランス** | 第2編3.16 exclusion zone | エンドエフェクタ（ハンド・グリッパ・吸着パッド） | 安全マージン／逃げ |
| `eed:0171#001` ★ | **インタフェース** | 第1編3.7 human-machine function allocation | 作業者・オペレータ・保全担当 | 人インタフェース／機械インタフェース |
| `eed:0172#001` ★ | **インタフェース** | 第1編3.8 interaction interface | HMI・操作盤 | 接点／窓口 |
| `eed:0173#001` ★ | **インタフェース** | 第4編3.4 work interface | 工程内場所（ステーション・セル） | 場所の役割 |
...
| ---: | :--- | :--- | :--- | ---: | ---: |
| 1 | **ワーク（被加工物・部品・製品）** | 工場の主役。加工され、運ばれ、組み立てられる当のもの | 形は第1編3.1・3.2、重さと重心は3.3、置き方は3.4。品番は第3編3.5、その1個は3.6、版は3.7、組み付けの関係は3.9、対称性は3.12、工程状態は3.14、履歴は3.18。品番と個体番号を束ねる上位概念が第3編3.4 ★。「表・裏」「正・逆」という粗い向きの区分は第2編3.19 ★ | 13 | 35 |
| 2 | **搬送トレー・パレット・コンテナ** | ワークを載せて運ぶ入れ物 | 置かれる場は第1編3.6、載せ方の約束は第3編3.15、積載から引き渡しまでの入れ子分解は第4編3.5。受け入れ条件の「向き不問」は第2編3.19 ★ | 4 | 3 |
| 3 | **治具（ジグ）・位置決めピン・突き当て面** | ワークを毎回同じ位置・姿勢に固定する道具 | 基準となる穴や面は第2編3.14、そこから立つ座標系は3.2、固定後の状態は第3編3.15 | 3 | 3 |
| 4 | **架台・ベースプレート・床** | 設備が据え付けられる土台 | 物差しの枠組みは第2編3.1、シーン全体の基準は3.9、親子関係は3.10、その両者の上位概念が3.7 ★ | 4 | 6 |
| 5 | **工程内場所（ステーション・セル）** | 役割を持って区切られた、名前のついた作業場所 | 場所そのものは第1編3.6、関与の接点は3.8 ★、通過するワークの状態は第3編3.14、境界での約束は3.15、機能的な役割は第4編3.4、内部の入れ子構造は3.5。現場で既定となる方向の枠（正面・上流側）は第2編3.18 ★ | 7 | 9 |
| 6 | **安全柵・ライトカーテン** | 人と機械を隔てる境界 | 人の関与のしかたは第1編3.7、立ち入り禁止空間は第2編3.16、床面の意味づけは第3編3.10 | 3 | 2 |
| 7 | **産業用ロボット（多関節マニピュレータ）** | 掴んで運ぶ・組み付ける腕 | 関節連鎖は第2編3.10、その計算は3.12・3.13、手先の位置と向きは3.5、向きの書き方は3.6 | 5 | 10 |
| 8 | **エンドエフェクタ（ハンド・グリッパ・吸着パッド）** | ロボットの手先。実際にワークに触れる部分 | 掴み方は第3編3.11、近づく向きは第2編3.17、触れてはいけない場所は3.16、持てる重さの判断材料は第1編3.3 | 4 | 8 |
| 9 | **コンベア・シュート** | ワークを一定方向へ流し続ける機構 | 機械側の関与として第1編3.7、場所の役割としては第4編3.4（変化対象＝配置または無変化）。「上流」「下流」は流れ方向に固定された絶対的枠として第2編3.18 ★ | 3 | 2 |
| 10 | **無人搬送車（AGV／AMR）** | 床を走ってワークを別の場所へ運ぶ | 走行して基準が動くため第2編3.8と3.9の区別が要になり、その上位概念が3.7 ★。機械側の関与として第1編3.7 | 4 | 0 |
| 11 | **工作機械・専用機（CNC、プレス、溶接機など）** | ワークの形そのものを変える設備 | 形を変える場所として第4編3.4（変化対象＝形状）、加工という操作の抽象は3.2、その上位概念が3.1 ★。軸ごとの移動量の見積もりは第2編3.11。「手前」「奥」は機械の正面を基準にした内在的枠として3.18 ★ | 5 | 3 |
...
| **〃** | PLC（シーケンサ） | II+III | 3 | 3 |
| **〃** | HMI・操作盤 | I+III | 4 | 1 |
| **型3 機能型** | 工程内場所（ステーション・セル） | I+II+III+IV | 7 | 9 |
| **〃** | 搬送トレー・パレット・コンテナ | I+II+III+IV | 4 | 3 |
...
**現場語数0の登場人物が3件ある**（センサ・無人搬送車・設計担当者）。概念としては語られているが、その登場人物について現場で使われる語が1語も採録されていない。2026-09-05 以前は6件だったが、同音異義と方向語の登録（★）により HMI・操作盤（インタフェース）・作業者（右・左・インタフェース）・コンベア（上流・下流）に初めて現場語が付いた。`eed:0145`（関与インタフェース）の現場語候補「パトライト」「操作盤」「置き治具」（第1編3.8 Note 4）が確定すれば、HMI・操作盤と工程内場所がさらに厚くなる。
...
| 第2編 | `原2.NN` → `3.N`（`3.7` の新設で `原2.07` 以降が1つ繰り下がる） | `3.18`・`3.19` は末尾に新設（2026-09-05。既存の採番は動かない） |
...
原辞書4章に対応します。**集計元は本編 附属書B（68件）であり、母数は集計項目ごとに異なります**（Clause 4.1）。
...
flowchart TB
    ...
    P2 --> F2["II-A 座標系そのもの 4<br/>II-B 位置・姿勢・隔たりという値 7<br/>II-C 変換という操作 2<br/>II-D 空間に設置される要素 4<br/>II-E 方向と向きの言語表現 2"]
...
| **方向参照枠** ★ | ほうこうさんしょうわく | direction reference frame | 第2編3.18 | —（新設） | 概念 | 業界一般 |
...
| **離散姿勢区分** ★ | りさんしせいくぶん | discrete orientation class | 第2編3.19 | —（新設） | 概念 | 独自 |
...
NOTE 1 **読みは原辞書8.1節から引き直した。** 本仕様書は原辞書の「読み」欄を不採録としているため（`decisions.md` D-2026-08-18-03）、索引を復元する際に原辞書側から取得する必要がある。新設提案9件と改称提案2件の読みは、原辞書に存在しないため本変換で与えた（`きかようそにたいするそうさ`・`くうかんひょうげん`・`あたい`・`しきべつし`・`かんよいんたふぇーす`・`もでるようそ`・`ようごがいねんたいおうのずれ`・`ほうこうさんしょうわく`・`りさんしせいくぶん`、および `ひときかいきのうはいぶん`・`じげんじょうしょうそうさ`）。**これらの読みも提案値**であり、確定時は原辞書8.1節と合わせること。
...
| **direction reference frame** ★ | 方向参照枠 | 第2編3.18 | —（新設） | 概念 | `eed:0152#001` |
| **discrete orientation class** ★ | 離散姿勢区分 | 第2編3.19 | —（新設） | 概念 | `eed:0153#001` |
...
NOTE **原辞書8.5節の20件に対し、増えたのは第1編3.8（関与インタフェース ★）と第2編3.19（離散姿勢区分 ★）の2件である。** 第2編3.18（方向参照枠 ★）は EXAMPLE 2 がロボティクスの実例であるため両分野 ◯ とした。新設提案のうち対比ペアの親概念6件は、下位概念の実例をそのまま継承するため両分野 ◯ とした。上位概念に固有の実例を作る必要はない——親概念の実例は、子概念の実例がそのまま親の実例でもあるからである。

**未確認が `CAD・ロボティクス` 側に偏っている（22件中20件）。** これは、この辞書が生産現場の語彙から育ってきたことの現れであり、附属書D.3 の型分けが示す偏り（ワークに記述が集中する）と同じ方向を向いている。
...
| 第2編3.18 ★ | 方向参照枠 | 工程内場所、コンベア、工作機械、作業者 |
| 第2編3.19 ★ | 離散姿勢区分 | ワーク、搬送トレー、PLC |
...
**附属書A を参照してください。** 現場語105語の全件が、代表形（概念）・主語（登場人物）・同義異表記とともに載っています。本編で唯一、索引の側ではなく附属書Aに置いた索引です——現場語は「引く対象」ではなく「引かれる語」であり、概念より下位の登録簿だからです（各編の附属書A と同じ位置づけ）。
...
本編の作成にあたり（2026-09-03 時点）、**全66エントリ・85現場語・20登場人物を1つの母数として機械照合しました。** 個々の編の変換では見えず、全編を数え直したときにだけ現れる不整合を5件、および推奨を1件検出しました。いずれも `backlog.md` §3 に F-16〜F-21 として登録しています。
