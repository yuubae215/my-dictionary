...
— refers-to 第2編3.14 (datum reference feature) ※データムフィーチャーは本概念の要素（面・穴・エッジ）から選定される。**部分関係（part-of の逆方向）への精密化は行わない**——基準フィーチャーは要素に役割を与えたものであり、その指定をすべて外してもトポロジー構造は変わらないため、部分にはあたらない（全編附属書C.5）
...
本編の8概念（うち3.8は本変換での新設提案）は、主題により3つのサブグループに区分されます（区分基準＝主題）。**3.8を含めた再検証は全編附属書F.4 で実施済みであり、I-A 2件／I-B 3件／I-C 3件＝8件、採番も連続して MECE が成立している**（`decisions.md` D-2026-09-03-03）。
...
| `eed:0171#001` ★ | 3.7 human-machine function allocation | インタフェース | 作業者・オペレータ・保全担当 | 「人インタフェース／機械インタフェース」という機能配分の軸名（第4編3.4 Note 4 の第1義）。同音異義：HMIの「インタフェース」は 3.8、工程内場所の「インタフェース」は第4編3.4 | 人インタフェース／機械インタフェース |
| `eed:0172#001` ★ | 3.8 interaction interface | インタフェース | HMI・操作盤 | 関与を媒介する物理的・情報的な接点（第4編3.4 Note 4 の第2義）。同音異義 | 接点／窓口 |

NOTE 1 3.7 (human-machine function allocation) および 3.8 (interaction interface) に固有の現場語（パトライト・操作盤・置き治具等）は未抽出（原辞書の記載を保存。存在しない現場語を創作しない）。3.8 の現場語候補は 3.8 の Note 4 参照。

NOTE 2（2026-09-05 追補） 現場語の一意性を「語」単位から「（語, 主語）の組」単位へ改めた（`decisions.md` D-2026-09-05-04）。これにより、本辞書が3義で用いている「インタフェース」（第4編3.4 Note 4、`decisions.md` D-2026-08-22-14）を主語違いの3行として登録できるようになった。本編の2行（★）と第4編の1行（`eed:0173`）がそれである。いずれも原辞書側での存在確認と発番の承認を要する（`backlog.md` P-06）。
...
NOTE 1 本表は軸2×軸3の空セル分析（原辞書4.5節）およびドメインロール別P/C逆引き（原辞書8.3節）の集計元である。**2026-09-03に全編附属書 `STD-EED-0001-0` が成立し、6編ぶんの附属書Bを統合した登録簿（66件）がそこに置かれた**（全編附属書B）。空セル分析は全編附属書F.5、P/C逆引きは全編附属書G.3 にある。
...
NOTE 1 原辞書の見本（STD-GEO-24159）はこのほか transforms、calibrated-from、aligns-with、mates-to の4型を定義するが、これらは座標変換・校正・作業一致・嵌合を表す型であり、Asset層（本編）には出現しない。第2編（Integration層）以降の変換で導入する。（2026-09-03追補：`transforms` は第2編・第4編で確定使用。`mates-to` は全6編を通じて確定使用ゼロのため**廃止**（第M編附属書C.1 NOTE 3、`decisions.md` D-2026-09-02-05）。`calibrated-from`・`aligns-with` は第2編で候補提示されたが、全編化時のレビューで**いずれも不採用**が確定した（全編附属書C.3、`decisions.md` D-2026-09-03-02）。**予告した4型のうち生き残ったのは `transforms` の1型だけである。**）

NOTE 2 本編で関係型を確定できたのは is-a 1件（3.4 → pose）のみで、他は保守的に refers-to とした。付与判断が原辞書の記載を超える箇所（3.2 の部分関係候補、3.5 の constrained-by 逆方向）は、各エントリの Note 2 内に「候補」「逆方向」として明示し、確定は全編化時のレビューに委ねる。（2026-09-03追補：`part-of`・`constrained-by` はいずれも第4編で確定使用が現れ、型としては確定した。本編の2箇所は候補のまま据え置く — 全編附属書C.2。）
...
原辞書1.2節の趣旨を保全する附属書です。登場人物（工場に物理的に実在するモノ・人）は概念とはメタレベルが異なるため Clause 3 には収めず、informative な附属書として維持します。以下は本編（Asset層）の8概念に接続する登場人物の抜粋です。
...
| HMI・操作盤 | 人が値を入れ、状況を見る画面 | 人側の関与区分は 3.7、接点そのものは 3.8（提示・操作の窓口） |

NOTE **登場人物の完全な一覧と双方向網羅性の検証は、全編附属書 `STD-EED-0001-0` の附属書D として実現した**（2026-09-03）。件数は**19件ではなく20件**である——原辞書1.2節は「19件」と書いているが、同節が列挙しているのも1.3節の表に並ぶのも20件であり、本 NOTE はその誤った件数を引き写していた（`backlog.md` F-18）。双方向の接続（すべての登場人物が最低1概念に、対象語彙のすべてのエントリが最低1登場人物に接続）は20件・50件の両方向で成立している（全編附属書D.2。2026-09-05 の新設2件を含めると52件）。
...
**（2026-09-03追補）本表のすべての行は、全編附属書 `STD-EED-0001-0` の成立をもって閉じた。実現状況の対照表は全編附属書E.1 にある。**
