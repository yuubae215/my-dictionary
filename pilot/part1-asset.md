...
— refers-to 3.13 (datum reference feature, 第2編) ※データムフィーチャーは本概念の要素（面・穴・エッジ）から選定されるため、全編化時に部分関係（part-of の逆方向）へ精密化する候補
— refers-to 3.1 (dimension-raising operation, 第4編) ※原4.01。第4編で名称を「掃引操作」から是正した（同編3.1 Note 3）
...
Note 2 to entry: Concept relations:
— refers-to 3.1 (declared value, 第3編)
— refers-to 3.2 (derived value, 第3編)
...
Note 2 to entry: Concept relations:
— is-a 3.5 (pose, 第2編) ※安定姿勢は、静的安定条件で拘束された姿勢の下位概念
...
Note 2 to entry: Concept relations:
— refers-to 3.5 (pose, 第2編), 3.14 (calibration reference, 第2編), 3.10 (symmetry rule, 第3編), 3.11 (v&v level, 第3編), 3.2 (measurement, 第4編)
...
— refers-to 3.3 (work interface, 第4編), 3.4 (nested IPO decomposition, 第4編), 3.8 (spatial element classification, 第3編), 3.12 (process state, 第3編), 3.13 (state expectation, 第3編)
...
— refers-to 3.3 (work interface, 第4編) ※3軸ファセット分類である同エントリと同型の構造であり、対比によって識別すべき2概念ではない。原辞書 参考表4-1 の「典型的な人インタフェース／機械インタフェース」2列は本概念の値を用いており、第4編附属書Fで列名を「典型的な機能配分」へ是正済み（原辞書側は未反映。`backlog.md` F-11）
...
— refers-to 3.13 (state expectation, 第3編) ※受け渡し接点の受け入れ条件を規定
— refers-to 3.13 (datum reference feature, 第2編) ※受け渡し接点の幾何的位置決めを担う
