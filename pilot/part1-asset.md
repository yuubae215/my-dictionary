...
NOTE **登場人物の完全な一覧と双方向網羅性の検証は、全編附属書 `STD-EED-0001-0` の附属書D として実現した**（2026-09-03）。件数は**19件ではなく20件**である——原辞書1.2節は「19件」と書いているが、同節が列挙しているのも1.3節の表に並ぶのも20件であり、本 NOTE はその誤った件数を引き写していた（`backlog.md` F-18）。双方向の接続（すべての登場人物が最低1概念に、対象語彙のすべてのエントリが最低1登場人物に接続）は20件・50件の両方向で成立している（全編附属書D.2）。
