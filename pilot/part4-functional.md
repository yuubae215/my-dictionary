...
| `eed:0169#001` ★ | 3.3 measurement | クリアランス | ワーク（被加工物・部品・製品） | 製品設計・公差・嵌合隙間として測る量（原辞書2.1節・機械設計文脈）。既存の現場語「クリアランス確認」（`eed:0139`）はこの量を確かめる行為。同音異義：エンドエフェクタの「クリアランス」は第2編3.16 | 隙間／公差 |
| `eed:0173#001` ★ | 3.4 work interface | インタフェース | 工程内場所（ステーション・セル） | 場所が果たす役割（3.4 Note 4 の第3義）。同音異義：作業者の「インタフェース」は第1編3.7、HMIの「インタフェース」は第1編3.8 | 場所の役割 |
...
NOTE 2 3.5 (nested IPO decomposition) に対応する現場語は未抽出（原辞書の記載を保存。存在しない現場語を創作しない）。3.4 (work interface) には2026-09-05 に同音異義「インタフェース」の第3義を登録した（★ `eed:0173`。現場語の一意性を「（語, 主語）の組」単位へ改めたことによる。`decisions.md` D-2026-09-05-04、`backlog.md` P-06）。3.3 の「クリアランス」（★ `eed:0169`）も同じ改定による登録で、原辞書2.1節の機械設計文脈の義にあたる。3.2 の現場語3語のうち「ライン作成」「スケッチ」は掃引ではないため、名称是正（3.2 Note 3）の直接の根拠となっている。
