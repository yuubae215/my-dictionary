...
- 原辞書の**恒久ID（EED-ID）は一切変更していない**。原辞書 第V部の概念採番（原 `5.01`）は本仕様書の `3.1` に1対1で対応する（附属書E.1）。
...
— refers-to 第C編3.1 (published language) ※本概念が閉じるスキーマは公開言語で書かれる。本概念は「非公式なやり取りを許さない」という統制（Business層）、公開言語はそこで交換される語彙と形式（Communication層）であり、EXAMPLE の JSON Schema・OPC UA情報モデルは後者の実現物である（2026-09-05 追加。`decisions.md` D-2026-09-05-08）

Note 3 to entry: 本エントリは実装非依存の段階に位置する。スキーマ化（原辞書6.2節の手順4）そのものは本辞書より下流の工程であり、本エントリが規定するのは「境界を閉じたスキーマとして扱う」という概念であって、特定のスキーマ言語ではない。設計レビュー（§3.1）は本エントリを Communication 層へ移す案も挙げたが、定義文が画定するのは統制であり Business 層が正しい。Communication 層の受け皿は第C編3.1（公開言語）として別に立てた（`backlog.md` B-09 クローズ）。
