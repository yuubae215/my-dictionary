...
- 原辞書の**恒久ID（EED-ID）は一切変更していない**。原辞書6.1節の採番（原 `M.01`〜`M.14`）は本仕様書の `3.1`〜`3.14` に欠番なく1対1で対応する（附属書E.1）。
...
— refers-to 第C編3.1 (published language) ※境界をまたいで交換されるデータの語彙と形式。境界の内側の語彙は違ってよいが、境界をまたぐデータは公開言語で書く（2026-09-05 追加）
...
**model element**
モデル要素
EED-ID: `eed:0150#001`（**新設提案**。Note 3参照）
...
EXAMPLE 1 （生産）設備制御チームの内側で、PLC のタグ名・HMI の表示・工程設計書が「搬入場所」「状態期待値」という同じ語を同じ意味で使っている状態。

EXAMPLE 2 （CAD）フロントエンド開発者とドメイン担当者が、ユーザーストーリーとクラス名に同じ語（「ワールド座標キャッシュ」）を使っている状態。

NOTE（2026-09-05 改訂） 原辞書の適用例は「この統合辞書そのもの。プログラムの変数名・UI表示・通信仕様書と辞書の用語を一致させて初めて機能する」であった。しかし本辞書は0章の4ドメイン（＝複数の境界づけられたコンテキスト、3.3）を**またいで**共有される語彙であり、それは DDD が Ubiquitous Language と区別する Published Language にあたる。この適用例は第C編3.1（公開言語）へ移し、本エントリにはコンテキストの**内側**の例を置いた（`decisions.md` D-2026-09-05-08）。具体例の差し替えは版を上げない（原辞書8.9節）。
...
— refers-to 第C編3.1 (published language) ※本概念をコンテキストの外へ公開した成果物（対象語彙、Communication層）。本辞書そのものはこちらにあたる（2026-09-05 追加）

Note 3 to entry: 本エントリは辞書について語る語彙（メタ語彙）である。本辞書そのものを本概念の適用例としていた旧記述は、本辞書が複数のコンテキストをまたぐ語彙である以上、第C編3.1（公開言語）の適用例として正しい。コンテキストの内側で「全員が同じ意味で使う」状態が本概念であり、それを境界の外へ公開した語彙が公開言語である。

### 3.9
**term–concept mismatch**
用語–概念対応のズレ
admitted term: 一義性・単名性の不成立
EED-ID: `eed:0151#001`（**新設提案**。Note 3参照）
...
— refers-to 第C編3.1 (published language) ※公開言語を定める作業は本作業を含む（逆参照。2026-09-05 追加）
...
キャッシュ鮮度の自己保証
admitted term: 鮮度保証／self-validating cache
DEPRECATED: freshness guarantee（原辞書 v0.17.0 の英語主名称。Note 4参照）
EED-ID: `eed:0056#001`
...
| 採番 | 用語 | EED-ID | 旧ID | Provider | Consumer | 語彙区分 | カバレッジ |
|---|---|---|---|---|---|---|---|
...
|---|---|---|---|---|
| 第2編3.8／3.9 ローカル空間／ワールド空間 | **第2編3.7 座標空間**（`eed:0146`。2026-09-05 に「空間表現」から改称し genus を「空間」に揃えた — `decisions.md` D-2026-09-05-09） | ○ | 業界一般（CG の object space／world space） | ○ 両者の差異的特性（基準に取る参照座標系）だけが示され、何の値なのかが示されない |
